...
export async function verifyApiKey(request: Request): Promise<boolean> {
  ...
  try {
    ...
    console.log(`提供的Key: ${providedKey.substring(0, 4)}****${providedKey.substring(providedKey.length - 4)}`);
    console.log(`配置的Key: ${config.api_key.substring(0, 4)}****${config.api_key.substring(config.api_key.length - 4)}`);
