...
export default function ApiLogsPage() {
  ...
  const handleCleanOldLogs = async () => {
    const days = prompt('请输入要保留的日志天数（例如：30 表示保留最近30天的日志）：');
    if (!days) return;
    ...
    const confirmed = window.confirm(
      `确定要删除 ${beforeDateStr} 之前的所有日志吗？此操作不可恢复！`
    );
    if (!confirmed) return;
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="bg-white rounded-lg shadow p-6 mb-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="text-2xl font-bold text-gray-900">{stats.totalCount.toLocaleString()}</div>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="text-sm text-gray-500">估算大小</div>
                  <div className={`text-2xl font-bold ${parseFloat(stats.estimatedSizeMB) > 10 ? 'text-red-600' : 'text-green-600'}`}>
                    {stats.estimatedSizeMB} MB
                  </div>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="text-sm text-gray-500">最大限制</div>
                  <div className="text-2xl font-bold text-gray-900">{stats.maxLogCount.toLocaleString()}</div>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="text-sm text-gray-500">自动清理阈值</div>
                  <div className="text-2xl font-bold text-gray-900">{stats.autoCleanThreshold.toLocaleString()}</div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleFilterChange('isRateLimitWarning', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                >
                  <option value="">全部</option>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                onClick={() => {
                  setFilters({ ip: '', endpoint: '', status: '', isRateLimitWarning: '', startDate: '', endDate: '' });
                  setPagination({ ...pagination, page: 1 });
                  fetchLogs();
                }}
                disabled={loading}
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {logs.map((log) => (
                      <tr key={log.id} className={log.is_rate_limit_warning ? 'bg-red-50' : ''}>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadge(log.response_status)}`}>
                            {log.response_status}
                          </span>
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
          ...
          <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <ul className="text-sm text-blue-800 space-y-1">
              ...
              <li>始终记录：错误请求（4xx/5xx）、限流警告、认证失败、慢请求（大于3秒）</li>
              <li>采样记录：每100次成功请求记录1次，或每10分钟记录1次，或随机采样（1%）</li>
