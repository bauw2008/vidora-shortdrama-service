...
export default function AdminPage() {
  ...
  const handleSync = async (type: 'full' | 'incremental' | 'resync' | 'continue') => {
    if (type === 'full') {
    ...
    try {
      ...
      if (res.ok) {
        const typeName = type === 'resync' ? '补充' : type === 'continue' ? '继续' : type === 'full' ? '完整' : '增量';
  ...
  const handleReset = async () => {
    ...
    const doubleConfirmed = window.confirm('⚠️ 最后确认：确定要清空所有数据吗？');
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.data.downloadText) {
          const blob = new Blob([data.data.downloadText], { type: 'text/plain;charset=utf-8' });
        ...
        alert(`测试同步完成！已获取 ${data.data.total} 个视频数据\n\n详细数据已下载为文本文件`);
      } else {
            ...
            <div className="px-4 py-5 sm:p-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className={syncStatus?.is_syncing ? 'text-yellow-600' : 'text-green-600'}>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">{syncStatus?.total_videos || 0}</span>
                </div>
              </div>
              ...
              <div className="mt-6 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 flex-wrap">
                ...
                {syncStatus?.is_syncing && syncStatus?.sync_type === 'full' && syncStatus?.current_page > 0 ? (
                  <button
            ...
            <div className="px-4 py-5 sm:p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div className="text-sm text-gray-500">管理视频数据 API 源</div>
                </button>
                <button
                ...
                >
                  ...
                  <div className="text-sm text-gray-500">管理一级分类和二级分类映射</div>
                </button>
                <button
                ...
                >
                  ...
                  <div className="text-sm text-gray-500">配置自动增量同步任务</div>
                </button>
                <button
                ...
                >
                  ...
                  <div className="text-sm text-gray-500">配置 /list 和 /detail 接口返回字段</div>
                </button>
                <button
                ...
                >
                  ...
                  <div className="text-sm text-purple-500">配置 API Key、认证开关和速率限制</div>
                </button>
                <button
                ...
                >
                  ...
                  <div className="text-sm text-red-500">管理被封禁的 IP 地址</div>
                </button>
                <button
                ...
                >
                  ...
                  <div className="text-sm text-green-500">查看 API 调用记录和统计</div>
                </button>
                <button
                  onClick={handleReset}
                  disabled={syncStatus?.is_syncing}
                ...
                >
                  ...
                  <div className="text-sm text-red-500">清空所有数据（⚠️ 危险操作）</div>
                </button>
                <button
                ...
                >
                  <div className="font-medium text-blue-900">数据备份与恢复</div>
                  <div className="text-sm text-blue-500">备份和恢复所有数据库数据</div>
                </button>
              </div>
            </div>
          </div>
